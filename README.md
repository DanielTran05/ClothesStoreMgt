...
* **Tầng GUI:** vào project `ClothesStore.GUI` -> Mở `App.config` -> Thay đổi giá trị tương tự ở thẻ `<connectionStrings>`; connectionString="Server=ten-server-tren-may;Database=clothesstoremgt;Trusted_Connection=True;TrustServerCertificate=True;
* **Tầng DAL:** vào project `ClothesStore.GUI` -> Mở file `ClothesstoremgtContext.cs` chỉnh lại tên server ở `dòng 50` thành tên server máy mình
...
3. Nhớ pull về lấy code mới nhất